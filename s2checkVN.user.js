...
; (function () {
    ...
    function wrapper(pluginInfo) {
        ...
        let firstPoint = null;
        let distanceLayer = L.layerGroup();
        ...
        function toggleDistanceMode() {
            ...
            if (distanceMode) {
                ...
                alert("📏 Distance mode ON: click 2 điểm hoặc 2 portal để đo.");
            } else {
                map.off('click', onMapClickMeasure);
                distanceLayer.clearLayers();
                map.removeLayer(distanceLayer);
                firstPoint = null;
                alert("❌ Distance mode OFF");
            }
        }

        // Khi click trên bản đồ
        function onMapClickMeasure(e) {
            handleSelectedPoint(e.latlng, "Point");
        }

        // Khi click vào portal/waypoint (Pokestop/Gym)
        function onPortalClickMeasure(e) {
            handleSelectedPoint(e.target._latlng, "Waypoint");
        }

        // Xử lý chọn điểm
        function handleSelectedPoint(latlng, label) {
            if (!distanceMode) return;

            if (!firstPoint) {
                firstPoint = latlng;
                L.marker(firstPoint, { title: label + " A" }).addTo(distanceLayer);
            } else {
                const secondPoint = latlng;
                L.marker(secondPoint, { title: label + " B" }).addTo(distanceLayer);

                // Vẽ đường
                const line = L.polyline([firstPoint, secondPoint], { color: 'blue' }).addTo(distanceLayer);

                // Tính khoảng cách
                const dist = firstPoint.distanceTo(secondPoint); // mét
                const distText = dist > 1000 ? (dist / 1000).toFixed(2) + " km" : dist.toFixed(0) + " m";

                // Popup ở giữa đoạn
                const midLat = (firstPoint.lat + secondPoint.lat) / 2;
                const midLng = (firstPoint.lng + secondPoint.lng) / 2;
                L.popup()
                    .setLatLng([midLat, midLng])
                    .setContent("📏 Khoảng cách: " + distText)
                    .openOn(map);
        ...
        // ==========================
        // Hook vào portal/waypoint
        // ==========================
        // sau khi các layer được thêm marker, bạn có thể gắn sự kiện click:
        function attachMeasureEventsToPortals() {
            Object.values(stopLayers).forEach(marker => {
                marker.off('click', onPortalClickMeasure);
                marker.on('click', onPortalClickMeasure);
            });
            Object.values(gymLayers).forEach(marker => {
                marker.off('click', onPortalClickMeasure);
                marker.on('click', onPortalClickMeasure);
            });
        }

        // Gọi attachMeasureEventsToPortals() sau khi addAllMarkers()
        const originalAddAllMarkers = thisPlugin.addAllMarkers;
        thisPlugin.addAllMarkers = function () {
            originalAddAllMarkers.apply(this, arguments);
            attachMeasureEventsToPortals();
        };

        // ==========================
        // Nút menu trong dialog
        // ==========================
        function addDistanceButton() {
            const btn = document.createElement("button");
            btn.textContent = "📏 Toggle Distance Mode";
            btn.style.margin = "5px";
            btn.onclick = toggleDistanceMode;
